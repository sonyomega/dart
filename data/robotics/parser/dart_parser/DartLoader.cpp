 ...
 */
dynamics::Skeleton* DartLoader::parseSkeleton( std::string _urdfFile,
 ...
 * @function parseRobot
 */
dynamics::Skeleton* DartLoader::parseRobot( std::string _urdfFile,
 ...
 */
dynamics::Skeleton* DartLoader::parseObject( std::string _urdfFile,
...
simulation::World* DartLoader::parseWorld( std::string _urdfFile ) {
  ...
  if( !worldInterface ) {
	...
	return world;
      ...
      dynamics::Joint* joint = object->getRoot()->getParentJoint();
	...
	return world;
      ...
      dynamics::Joint* joint = robot->getRoot()->getParentJoint();
      joint->getTransform(0)->getDof(0)->setValue(pose(0));
      joint->getTransform(0)->getDof(1)->setValue(pose(1));
      joint->getTransform(0)->getDof(2)->setValue(pose(2));
      joint->getTransform(1)->getDof(0)->setValue(pose(5));
 ...
 */
dynamics::Skeleton* DartLoader::modelInterfaceToSkeleton( boost::shared_ptr<urdf::ModelInterface> _model,
								  std::string _rootToSkelPath ) {
  ...
  dynamics::BodyNode *node, *rootNode;
  dynamics::Joint *joint, *rootJoint;
  
  /** Create new skeleton object */
  mSkeleton = new dynamics::Skeleton();
  ...
  // Push parents first
  std::list<dynamics::BodyNode*> nodeStack;
  dynamics::BodyNode* u;
  ...
  while( !nodeStack.empty() && numIter < mNodes.size() ) {
    ...
    for( int idx = 0; idx < u->getNumChildJoints(); ++idx ) {
      nodeStack.push_back( (dynamics::BodyNode*)( u->getChildNode(idx) ) );
    }
    numIter++;
  } // end while
 ...
 */
dynamics::Skeleton* DartLoader::modelInterfaceToRobot( boost::shared_ptr<urdf::ModelInterface> _model,
						    std::string _rootToRobotPath ) {
  ...
  dynamics::Skeleton* mRobot;
  dynamics::BodyNode *node, *rootNode;
  dynamics::Joint *joint, *rootJoint;
  
  /** Create new robot object */
  mRobot = new dynamics::Skeleton();
 ...
 */
dynamics::Skeleton* DartLoader::modelInterfaceToObject( boost::shared_ptr<urdf::ModelInterface> _model,
						      std::string _rootToObjectPath ) {
  ...
  dynamics::Skeleton* mObject;
  dynamics::BodyNode *node, *rootNode;
  dynamics::Joint *joint, *rootJoint;

  mObject = new dynamics::Skeleton();
  ...
  for( unsigned int i = 0; i < mNodes.size(); ++i ) {
    // Add nodes to the object
      mObject->addBody( mNodes[i] );
  }
  
  // Init object (skeleton)
  mObject->initDynamics();
 ...
 * @function getNode
 */
dynamics::BodyNode* DartLoader::getNode( std::string _nodeName ) {
