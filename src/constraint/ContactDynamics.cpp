...
ContactDynamics::ContactDynamics(const std::vector<dynamics::Skeleton*>& _skels,
                                 double _dt,
                                 double _mu,
                                 int _d)
    : mSkels(_skels),
...
void ContactDynamics::initialize() {
    ...
    for (int i = 0; i < getNumSkels(); i++) {
        ...
        for (int j = 0; j < nNodes; j++) {
            ...
            if (node->getCollisionShape()->getShapeType() != dynamics::Shape::P_UNDEFINED)
            {
...
void ContactDynamics::addSkeleton(dynamics::Skeleton* _newSkel) {
    //
...
MatrixXd ContactDynamics::getJacobian(dynamics::BodyNode* node, const Vector3d& p) {
